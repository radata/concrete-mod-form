...
class Concrete5_Controller_Block_Form extends BlockController {
    ...
    public function getDefaultThankYouMsg() {
        return t("<img title='ok' src='http://bizztemp.nl/ok.png' width='30px' height='30px'></img>");
    }
    ...
    function duplicate($newBID) {
        ...
        if(count($row)>0){
            ...
            while( $row=$rs->fetchRow() ){
                $v=array($newQuestionSetId,intval($row['msqID']), intval($newBID), $row['question'],$row['inputType'],$row['options'],$row['position'],$row['width'],$row['height'],$row['required'],$row['Lpl'],$row['Lnl'],$row['Les'],$row['optionsLpl'],$row['optionsLnl'],$row['optionsLes'],$row['Lregex'],$row['Lmax'],$row['Lmin'],$row['LminL'],$row['LmaxL']);
                $sql= "INSERT INTO {$this->btQuestionsTablename} (questionSetId,msqID,bID,question,inputType,options,position,width,height,required,Lpl,Lnl,Les,optionsLpl,optionsLnl,optionsLes,Lregex,Lmax,Lmin,LminL,LmaxL) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
                $db->Execute($sql, $v);
            }
...
//users submits the completed survey
    function action_submit_form() {
        ...
        foreach($rows as $row){
            ...
            // validation RA
            $idcollor = 'Question' . $row['msqID'];

            if($_POST['Question' . $row['msqID']] != null or intval($row['required'])==1){
                $Lregex = $row['Lregex'];
                $notCompleted=0;
                if ($row['inputType']=='date'){
                    if(strlen($_POST['Question' . $row['msqID']]) <= $LminL or strlen($_POST['Question' . $row['msqID']]) >= $row['LmaxL']){
                        $notCompleted=1;
                        $idcollorresult = $idcollorresult . '$("#' . $idcollor . '").css({"background-color": "rgb(255, 183, 183)"});' . "\r\n";
                    }
                }
                ...
                if($row['inputType']=='checkboxlist' or $row['inputType']=='checkboxlist_hidden'){
                    if(!isset($_POST['Question' . $row['msqID'] . '_0'])){
                        $notCompleted = 1;
                        $idcollorresult = $idcollorresult . '$("#' . $idcollor . '").css({"background-color": "rgb(255, 183, 183)"});' . "\r\n";                   //strlen(0$val) ){
                    }
                }
                if($row['inputType']=='fileupload'){
                    if(!isset($_FILES['Question'.$row['msqID']]) or !is_uploaded_file($_FILES['Question'.$row['msqID']]['tmp_name'])){
                        $notCompleted=1;
                        $idcollorresult = $idcollorresult . '$("#' . $idcollor . '").css({"background-color": "rgb(255, 183, 183)"});' . "\r\n";
                    }
                }
                if($Lregex == "number"){
                    if(strlen($_POST['Question' . $row['msqID']]) <= $row['LminL'] or strlen($_POST['Question' . $row['msqID']]) >= $row['LmaxL'] or $_POST['Question' . $row['msqID']] <= $row['Lmin'] or $_POST['Question' . $row['msqID']] >= $row['Lmax']){
                        $notCompleted=1;
                        $idcollorresult = $idcollorresult . '$("#' . $idcollor . '").css({"background-color": "rgb(255, 183, 183)"});' . "\r\n";
                    }
                }
                if($Lregex == "text"){
                    if(strlen($_POST['Question' . $row['msqID']]) <= $row['LminL'] or strlen($_POST['Question' . $row['msqID']]) >= $row['LmaxL']){
                        $notCompleted=1;
                        $idcollorresult = $idcollorresult . '$("#' . $idcollor . '").css({"background-color": "rgb(255, 183, 183)"});' . "\r\n";
                    }
                }
                if($row['inputType']=='field' && $Lregex <> "text" && $Lregex <> "number"){
            ...
            if($notCompleted) $errors['CompleteRequired'] = t(" ");
        }
        ...
        if(count($errors)){
            $this->set('formResponse', t('<img title="error" src="http://bizztemp.nl/error.png" width="30px" height="30px"></img>') );
            $this->set('errors',$errors);
            $this->set('Entry',$E);
            if(isset($idcollorresult)){$this->set('idcollorresponse', t('<script type="text/javascript">' . "\r\n" .'$(function(){'  . "\r\n" . $idcollorresult  . '});</script>' . "\r\n"));}else{$this->set('idcollorresponse', t(''));}
        }else{ //no form errors
